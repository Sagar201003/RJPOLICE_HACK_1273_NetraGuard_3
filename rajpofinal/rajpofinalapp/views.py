...
import random
from .models import FilesUpload,criminal_table,Video
...
def vid_inp(request):
    ...
    # else:
        return video_feed(request, 'rajpofinalapp/tmali.mp4')
...
# Find the element with the maximum count
                ...
                if len(testlist)==4 and stop==False:
                        ...
                        data = {'name': [], 'criminal_id': [], 'image': [],'case': [],'Inperiod': [],'area': [], }  # Add other fields as needed
    ...
    # with open(f"out_{timestamp}.mp4", "rb") as video_file:
    #     video_record = Video(title=f"Video {timestamp}", video_file=video_file)
    #     video_record.save()  
    if os.path.exists(video_path):
        try:
            with open(video_path, "rb") as video_file:
                # Save video file as a Video model object
                video_record = Video(title=f"Video {timestamp}", video_file=video_file)
                video_record.save()
        except Exception as e:
            print(f"Error while saving video record: {e}")
    else:
...
# def update_anomaly_alert(top_class):
#     # Load the template
#     template = loader.get_template('home.html')

#     # Render the template with the updated anomaly_class
#     rendered = template.render({'anomaly_class': top_class})

#     # Return the updated HTML as an HttpResponse
